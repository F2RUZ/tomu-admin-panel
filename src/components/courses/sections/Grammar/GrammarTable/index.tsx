// src/components/courses/sections/Grammar/GrammarTable/index.tsx
"use client";
...
import { Grammar } from "@/types/grammar.types";
import GrammarService from "@/services/grammarService";
import { useSnackbarStore } from "@/store/snackbarStore";
import GrammarModal from "@/components/courses/sections/Grammar/GrammarModal";
...
export default function GrammarTable({ courseId }: GrammarTableProps) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<Grammar | null>(null);
  const [videoTarget, setVideoTarget] = useState<Grammar | null>(null);

  // ── useQuery ─────────────────────────────────────────────────────────────
  const { data, isLoading } = useQuery({
    queryKey: ["grammars", courseId],
    queryFn: () => GrammarService.getByCourse(courseId),
    select: (res) => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      useSnackbarStore.getState().success("Grammar o'chirildi");
      queryClient.invalidateQueries({ queryKey: ["grammars", courseId] });
  ...
  const filtered = useMemo(() => {
    ...
    return grammars.filter((g) => g.title.toLowerCase().includes(q));
  }, [search, grammars]);
  ...
  const cols = ["#", "Grammar nomi", "Tartib", "Davomiylik", "Hajm", "Amallar"];
  ...
  return (
    <Box>
      <PageHeader
        title="Grammar darslari"
        subtitle={`Jami ${grammars.length} ta grammar`}
        action={{
          label: "Yangi grammar",
          onClick: () => {
            setEditData(null);
                    ...
                    <EmptyState
                      title={
                        search
                          ...
                          : "Hali grammar darslari yo'q"
                      }
                      description={
                        search
                          ? "Boshqa kalit so'z bilan qidiring"
                          : "Birinchi grammar darsini yarating"
                      }
                      action={
                        !search
                          ? {
                              label: "Grammar yaratish",
                  ...
                  >
                    ...
                    <td style={tdStyle}>
                      ...
                      >
                        <Tooltip title="Videoni ko'rish" placement="top" arrow>
                          <Box
                            onClick={() => setVideoTarget(grammar)}
                            sx={{
                              width: 32,
                              height: 32,
                              borderRadius: "8px",
                    ...
                    <td style={tdStyle}>
                      ...
                      >
                        ...
                        >
                          {formatDuration(grammar.duration)}
                        </Typography>
                      </Box>
                    </td>
      ...
      <ConfirmModal
        open={!!deleteTarget}
        title="Grammerni o'chirish"
        message={`"${deleteTarget?.title}" grammerni o'chirasizmi? Bu amalni qaytarib bo'lmaydi.`}
